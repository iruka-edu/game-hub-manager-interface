...
import type { QCDecision } from '@/src/models/QcReport';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!qcReport) {
      return NextResponse.json(
        { error: 'No QC report found for this version. Please run QA testing first.' },
        { status: 404 }
      );
    }
    ...
    if (decision === 'pass') {
      ...
      if (!qa04Pass) {
        return NextResponse.json(
          { error: 'Cannot pass QC when QA-04 idempotency test failed', qa04Pass },
          { status: 400 }
        );
      }
    ...
    try {
      ...
      const updatedVersion = await versionRepo.updateStatus(versionId, newStatus);
      ...
      await GameHistoryService.recordQcResult(game._id.toString(), user, decision, note);
      ...
      AuditLogger.log({
        ...
        changes: [{ field: 'status', oldValue: version.status, newValue: newStatus }],
        metadata: {
          gameId: game.gameId,
      ...
      return NextResponse.json({
        ...
        qaResults: {
          qa01: { pass: qa01Pass },
          qa02: { pass: qa02Pass },
          qa03: { auto: updatedQAResults.qa03.auto, manual: updatedQAResults.qa03.manual },
          qa04: { pass: qa04Pass },
          overall: decision,
        },
      });
