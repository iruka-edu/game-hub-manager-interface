...
import { VersionStateMachine } from '@/src/lib/version-state-machine';
...
import { SelfQAChecklist } from '@/components/game/SelfQAChecklist';
import { GameActions } from '@/components/game/GameActions';
import { WorkflowTimeline } from '@/components/game/WorkflowTimeline';
...
export default async function GameDetailPage({ params, searchParams }: Props) {
  ...
  const { tab = 'info', error: errorParam, status: statusParam } = await searchParams;
  ...
  const canApprove = (isCTO || isCEO || isAdmin) && currentStatus === 'qc_passed';
  ...
  if (errorParam === 'cannot_edit') {
    const statusText =
      statusParam === 'uploaded' ? 'đang chờ QC' :
      statusParam === 'qc_processing' ? 'đang được QC kiểm tra' :
      statusParam === 'qc_passed' ? 'đã qua QC' :
      statusParam === 'approved' ? 'đã được duyệt' :
      statusParam === 'published' ? 'đã xuất bản' :
      statusParam === 'archived' ? 'đã lưu trữ' : 'không thể chỉnh sửa';
    ...
    <div className="p-8">
      ...
      {errorMessage && (
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-semibold text-red-800">Không thể thực hiện thao tác</h4>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-2xl font-bold text-slate-900">{game.title || game.gameId}</h1>
      ...
      {needsSelfQaWarning && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8 flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-semibold text-amber-800">Self-QA chưa hoàn tất</h4>
            <p className="text-sm text-amber-700 mt-1">
              Bạn cần tích chọn và lưu tất cả mục trong checklist Self-QA bên dưới trước khi có thể gửi cho QC kiểm tra.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              <h3 className="font-semibold text-slate-900 mb-4">Trạng thái hiện tại</h3>
              <div className="flex items-center gap-3 mb-4">
                <StatusChip status={currentStatus} />
              </div>
              <p className="text-sm text-slate-600">
                {currentStatus === 'draft' && 'Game đang ở trạng thái nháp. Dev có thể chỉnh sửa và gửi QC khi sẵn sàng.'}
                {currentStatus === 'uploaded' && 'Game đang chờ QC kiểm tra chất lượng.'}
                {currentStatus === 'qc_processing' && 'QC đang tiến hành kiểm tra game này.'}
                {currentStatus === 'qc_passed' && 'Game đã qua QC và đang chờ CTO/CEO phê duyệt.'}
                {currentStatus === 'qc_failed' && 'Game cần được Dev chỉnh sửa theo feedback của QC.'}
                {currentStatus === 'approved' && 'Game đã được duyệt và đang chờ Admin xuất bản.'}
                {currentStatus === 'published' && 'Game đã được xuất bản và học sinh có thể chơi.'}
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-6">
                ...
                <dl className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <dd className="font-mono text-slate-900">{versionData.version}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-slate-500">Cập nhật</dt>
                    <dd className="text-slate-900">
                      {new Date(versionData.updatedAt).toLocaleDateString('vi-VN')}
                    </dd>
                  </div>
                </dl>
              </div>
      ...
      {tab === 'history' && (
        <div className="mt-8 bg-white rounded-xl border border-slate-200 p-6">
          <h3 className="font-semibold text-slate-900 mb-4">Lịch sử thay đổi</h3>
          {history.length > 0 ? (
                ...
                <div key={index} className="flex gap-4 pb-4 border-b border-slate-100 last:border-0">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      {new Date(item.timestamp).toLocaleString('vi-VN')} - {item.user}
                    </p>
                  </div>
                </div>
              ))}
          ...
          {qcReports.length > 0 ? (
            <div className="space-y-4">
              {qcReports.map((report: any) => (
                <div key={report._id.toString()} className="p-4 border border-slate-200 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-sm font-medium ${report.decision === 'pass' ? 'text-green-600' : 'text-red-600'}`}>
                  ...
                  {report.notes && <p className="text-sm text-slate-600">{report.notes}</p>}
                </div>
              ))}
            </div>
          ) : (
