...
async function runBackendCheckpoint() {
  ...
  try {
    ...
    try {
      ...
      if (
        mockReportInput.gameId &&
        mockReportInput.versionId &&
        mockReportInput.qcUserId
      ) {
    ...
    try {
      ...
      testResults.push({
        test: "TestRunner Factory Functions",
        status: "PASS",
      });
      ...
      const normalizedResults = await TestRunnerService.normalizeResults(
        validResults
      );
      ...
      const idempotencyResult = await TestRunnerService.validateIdempotency(
        "test-game",
        "test-version"
      );
    ...
    try {
      ...
      if (
        ...
        typeof decisionApi.POST === "function"
      ) {
