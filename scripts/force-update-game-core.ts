...
async function forceUpdateGameCore() {
  ...
  if (!process.env.GITHUB_TOKEN) {
    ...
    try {
      const installedPackagePath = join(process.cwd(), 'node_modules', '@iruka-edu', 'game-core', 'package.json');
  ...
  try {
    ...
    try {
      const installedPackagePath = join(process.cwd(), 'node_modules', '@iruka-edu', 'game-core', 'package.json');
      const installedPackage = JSON.parse(readFileSync(installedPackagePath, 'utf8'));
      console.log(`✅ Successfully installed version: ${installedPackage.version}`);
    } catch (error) {
      console.log('✅ Installation completed (could not read installed version)');
    }
