...
export function QCInboxClient({ allGames, userRoles }: QCInboxClientProps) {
  ...
  const filteredGames = useMemo(() => {
    return allGames
      .filter((game) => {
        // Tab filter: Pending (No live version) vs Published (Has live version)
        ...
        if (searchTerm) {
          ...
          const matchesTitle = game.title.toLowerCase().includes(term);
          const matchesId = game.gameId.toLowerCase().includes(term);
          const matchesLesson = game.tags?.level?.toLowerCase().includes(term);
          if (!matchesTitle && !matchesId && !matchesLesson) return false;
        }

        // Subject filter
        if (filterSubject !== "all" && game.subject !== filterSubject)
          return false;
        ...
        // Issue filter
        const openIssues = game.qcIssues.filter(
          (i) => i.status !== "closed",
        ).length;
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-6">
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Tìm tên game, ID, hoặc bài học..."
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {Object.entries(SUBJECT_MAP).map(([val, label]) => (
                  ...
                  <td colSpan={5} className="px-6 py-32 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-slate-500 max-w-xs mx-auto">
                        Vui lòng điều chỉnh lại bộ lọc hoặc từ khóa tìm kiếm của
                        bạn.
                      </p>
                      <button
                        onClick={() => {
                  ...
                  >
                    <td className="px-6 py-6">
                      <div className="flex items-center gap-5">
                        <div className="w-24 h-16 bg-slate-100 rounded-xl overflow-hidden border border-slate-200 shrink-0 relative shadow-sm group-hover:shadow transition-shadow">
                        ...
                        <div className="min-w-0">
                          <h4 className="font-bold text-slate-900 truncate text-[15px] group-hover:text-indigo-600 transition-colors">
                            {game.title}
                          </h4>
                    ...
                    <td className="px-6 py-6">
                      <div className="space-y-2">
                        ...
                        <div className="text-[11px] text-slate-500 font-bold truncate max-w-[180px] group-hover:text-slate-700 transition-colors">
                          <span className="text-slate-400">Bài:</span>{" "}
                          {game.tags?.level || "Chưa gắn"}
                        </div>
                      </div>
                    </td>
                        ...
                        {game.qcIssues.length > 0 ? (
                          <>
                            ...
                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-4">
                              Tổng: {game.qcIssues.length} issue
                            </div>
                          </>
                        ) : (
                          <div className="flex items-center gap-2">
