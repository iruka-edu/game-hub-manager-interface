 ...
 * 1. Individual env vars: GAME_HUB_MANAGER_GCLOUD_CLIENT_EMAIL, GAME_HUB_MANAGER_GCLOUD_PRIVATE_KEY (recommended for Vercel)
...
function createStorageClient(): Storage {
  const options: StorageOptions = {
    projectId: process.env.GAME_HUB_MANAGER_GCLOUD_PROJECT_ID,
  };

  // Method 1: Individual credential fields (recommended for Vercel/.env)
  if (
    process.env.GAME_HUB_MANAGER_GCLOUD_CLIENT_EMAIL &&
    process.env.GAME_HUB_MANAGER_GCLOUD_PRIVATE_KEY
  ) {
    options.credentials = {
      client_email: process.env.GAME_HUB_MANAGER_GCLOUD_CLIENT_EMAIL,
      private_key: process.env.GAME_HUB_MANAGER_GCLOUD_PRIVATE_KEY.replace(
        /\\n/g,
        "\n",
  ...
  // No credentials found
    ...
    console.warn(
      "[GCS] No credentials found! Set GAME_HUB_MANAGER_GCLOUD_CLIENT_EMAIL + GAME_HUB_MANAGER_GCLOUD_PRIVATE_KEY",
    );
  }
...
const bucketName =
  process.env.GAME_HUB_MANAGER_GCLOUD_BUCKET_NAME || "iruka-edu-mini-game";
 ...
 */
export const getFileContent = async <T = unknown>(
  path: string,
): Promise<T | null> => {
  try {
...
export const uploadGameFiles = async (
  ...
  try {
    ...
    const indexHtmlPaths = allFiles.filter(
      (path) =>
    ...
    if (pathParts.length > 1) {
      ...
      console.log(
        `[GCS] Tìm thấy index.html tại: ${indexPath}, sử dụng root: ${rootPath}`,
      );
    } else {
    ...
    for (const [relativePath, zipEntry] of Object.entries(zip.files)) {
      ...
      const fileRelativePath = rootPath
    ...
    if (uploadItems.length === 0) {
      throw new Error(
        "Không có file nào để upload sau khi xác định root directory",
      );
    }

    // Upload all files in parallel
    console.log(
      `[GCS] Đang tải lên ${uploadItems.length} file từ ${fileName} (root: ${rootPath || "ZIP root"})...`,
    );

    const results = await uploadBufferBatch(
      uploadItems,
      5,
