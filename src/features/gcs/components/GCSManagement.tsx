...
import { useGCSFolders, useDeleteGCSFile, useRefreshGCS } from "../hooks/useGCS";
import type { GCSGameFolder, GCSFile, GCSFolderFilter, GCSSortField, GCSSortOrder } from "../types";
...
export function GCSManagement({ isAdmin }: GCSManagementProps) {
  ...
  const handleDeleteSelected = () => {
    setDeleteConfirm({ isOpen: true, folder: null, file: null, isMultiple: true });
  };
  ...
  const confirmDelete = async () => {
    if (deleteConfirm.isMultiple) {
      ...
      await deleteFileMutation.mutateAsync(`games/${deleteConfirm.folder.gameId}/`);
    } else if (deleteConfirm.file) {
      // Delete single file
      await deleteFileMutation.mutateAsync(deleteConfirm.file.name);
    }
    
    setDeleteConfirm({ isOpen: false, folder: null, file: null, isMultiple: false });
  };
  ...
  if (!isAdmin) {
    return (
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <p className="text-slate-500">Chỉ Admin mới có thể truy cập quản lý GCS.</p>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div className="bg-white rounded-lg border border-slate-200 p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-slate-600">Game folders</p>
                <p className="text-2xl font-bold text-slate-900">{gcsData.stats.totalFolders}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-slate-900">{gcsData.stats.totalFiles}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-slate-600">Có trong DB</p>
                <p className="text-2xl font-bold text-slate-900">{gcsData.stats.inDatabase}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-slate-600">Folders rác</p>
                <p className="text-2xl font-bold text-slate-900">{gcsData.stats.orphaned}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-slate-900">{formatFileSize(gcsData.stats.totalSize)}</p>
              </div>
            </div>
          </div>
        </div>
                  ...
                  <tr key={folder.gameId} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <input
                        ...
                        onChange={(e) => handleSelectFolder(folder.gameId, e.target.checked)}
                        className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      />
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <tr>
                      <td colSpan={10} className="px-6 py-4 bg-gray-50">
                        <div className="space-y-2">
                          <h4 className="font-medium text-gray-900">Files trong folder {folder.gameId}:</h4>
          ...
          <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div className="fixed inset-0 transition-opacity" aria-hidden="true">
              <div
                ...
                onClick={() => setDeleteConfirm({ isOpen: false, folder: null, file: null, isMultiple: false })}
              ></div>
            </div>
